...
test_api() ->
  ...
  R5 = post_obj("/api/account/1/project/5/user/", User, [2,5,7]),
  {R0, R1, R2, R3, R4, R5}.
...
push(Path, Obj, Method) ->
  Body = jsx:encode(Obj),
  Hdrs = [{"content-type", "application/json"}],
  ibrowse:send_req("http://127.0.0.1:8081" ++ Path, Hdrs, Method, Body).

pull(Path, Method) ->
  Hdrs = [{"accept", "application/json"}],
  ibrowse:send_req("http://127.0.0.1:8081" ++ Path, Hdrs, Method).
