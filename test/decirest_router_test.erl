...
mock_name(Mod, Name)  ->
  meck:expect(Mod, name, 0, Name).
mock_paths(Mod, Paths) ->
  meck:expect(Mod, paths, 0, Paths).
mock_child_of(Mod, Parents) ->
  meck:expect(Mod, child_of, 0, Parents).
mock_ident(Mod, Ident) ->
  meck:expect(Mod, ident, 0, Ident).
