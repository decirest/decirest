...
-spec init(_, map()) ->
              {cowboy_rest,
               ...
               #{rstate := #{},
                 _ => _}}.
init(Req, State) -> decirest_handler_lib:init_rest(Req, State).

-spec is_authorized(_,
                    #{module := atom(),
                      _ => _}) ->
                       any().
is_authorized(Req, State) -> decirest_handler_lib:is_authorized(Req, State).

-spec forbidden(_,
                #{module := atom(),
                  _ => _}) ->
                   any().
forbidden(Req, State) -> decirest_handler_lib:forbidden(Req, State).
...
allow_missing_post(Req, State = #{module := Module}) ->
    decirest:do_callback(Module,
                         ...
                         Req,
                         State,
                         fun allow_missing_post_default/2).

allow_missing_post_default(Req, State) -> {false, Req, State}.

-spec allowed_methods(_,
                      #{module := atom(),
                        _ => _}) ->
                         any().
allowed_methods(Req, State = #{module := Module}) ->
    ...
    case decirest_req:method(Req) of
        ...
        _ -> {Methods, Req1, State1}
    end.

-spec allowed_methods_default(_,
                              #{module := atom(),
                                _ => _}) ->
                                 {[<<_:24, _:_*8>>, ...],
                                  _,
...
allowed_methods_default(Req, State = #{module := Module}) ->
    ...
    ExportMappingList = [{{persist_data, [2, 3]}, [<<"POST">>]}],
    Methods =
        decirest_handler_lib:export_to_methods(Module, ExportMappingList, DefaultMethods),
    {Methods, Req, State}.

options(Req, State) -> decirest_handler_lib:options(Req, State).
...
-spec from_fun_default(#{path := _,
                         _ => _},
                       #{module := atom(),
                         _ => _}) ->
                          {false | stop | {true, binary()}, map(), _}.
...
-spec to_fun_default(_,
                     #{module := atom(),
                       _ => _}) ->
                        any().
to_fun_default(Req, State) -> to_json(Req, State).
...
to_html_default(Req, State = #{module := Module}) ->
    ...
    Title = Module:name(),
    PK = decirest:module_pk(Module),
    Context = [{data_pk, PK}, {title, Title}, {collection_data, Json}],
...
filter_data_on_pk(Data, Req, State = #{module := Module}) ->
    ...
    PK = case decirest_handler_lib:is_exported(Module, data_pk, 0) of
             true -> Module:data_pk();
             false -> id
         end,
...
get_self_url(Path, Data, PKVal) ->
    case maps:get(self_url_partials, Data, undefined) of
        undefined -> decirest:pretty_path([Path, "/", decirest:t2b(PKVal)]);
        {Replace, Add} -> replace_and_add(Replace, Add, Path)
    end.

replace_and_add(Replace, Add, Path) ->
    case hd(string:split(Path, <<"/", Replace/binary>>)) of
        [] -> error(wrong_self);
        SelfBase -> <<SelfBase/binary, "/", Add/binary>>
    end.

-spec resource_exists(_,
                      #{module := atom(),
                        _ => _}) ->
                         any().
resource_exists(Req, State = #{module := Module}) ->
    decirest:apply_with_default(Module,
                                resource_exists,
                                [Req, State],
                                fun resource_exists_default/2).

-spec resource_exists_default(_, map()) ->
                                 {_,
                                  _,
                                  #{mro_call := boolean(),
                                    _ => _}}.
resource_exists_default(Req, State = #{mro_call := true}) -> {true, Req, State};
