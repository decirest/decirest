...
init_rest(Req, State) -> decirest:do_callback(init, Req, State, fun init_rest_default/2).

init_rest_default(Req, State) ->
    {cowboy_rest,
     Req#{bindings => decirest_query:get_bindings(Req, State)},
     State#{rstate => #{}}}.
...
is_authorized_default(Req, State) -> decirest_auth:is_authorized(Req, State).
...
-spec content_types_accepted(_,
                             #{module := atom(),
                               _ => _}) ->
                                any().
content_types_accepted(Req, State = #{module := Module}) ->
    decirest:do_callback(Module,
                         content_types_accepted,
                         Req,
                         State,
                         fun content_types_accepted_default/2).
...
export_to_methods(Module, ExportMappingList, DefaultMethods) ->
    lists:foldl(fun({{Function, Arity}, Methods}, Acc) ->
                   case is_exported(Module, Function, Arity) of
                       true -> lists:append([Methods, Acc]);
                       false -> Acc
                   end
                end,
                DefaultMethods,
...
is_exported(Module, Function, ArityList) when is_list(ArityList) ->
    lists:any(fun(R) -> R end, [is_exported(Module, Function, Arity) || Arity <- ArityList]);
is_exported(Module, Function, Arity) -> erlang:function_exported(Module, Function, Arity).

fetch_data(Req, State = #{module := Module}) ->
    Bindings = decirest_req:bindings(Req),
    case is_exported(Module, fetch_data, 3) of
        true -> Module:fetch_data(Bindings, Req, State);
        false -> Module:fetch_data(Bindings, State)
    end.

delete_data(Req, State = #{module := Module}) ->
    Bindings = decirest_req:bindings(Req),
    case is_exported(Module, delete_data, 3) of
        true -> Module:delete_data(Bindings, Req, State);
        false -> Module:delete_data(Bindings, State)
    end.

validate_action(Body, Req, State) -> validate_action_parts(Body, Req, State).

validate_payload(Body, Req, State) -> validate_payload(normal, Body, Req, State).

validate_payload(_Type, Body, Req, State = #{module := Module}) ->
    case is_exported(Module, validate_payload, 3) of
        true -> Module:validate_payload(Body, Req, State);
        false ->
            case is_exported(Module, validate_payload, 2) of
                true -> Module:validate_payload(Body, State);
                false ->
...
validate_bin({Body, Req, State = #{module := Module}}) ->
    Res = decirest:apply_with_default(Module,
                                      validate_bin,
                                      [Body, Req, State],
                                      fun validate_bin_default/3),
    epipe_resp(Res, Req, State).

validate_bin_default(Body, _Req, _State) -> {ok, Body}.

to_term({Body, Req, State = #{module := Module}}) ->
    Res = decirest:apply_with_default(Module,
                                      to_term,
                                      [Body, Req, State],
                                      fun to_term_default/3),
    epipe_resp(Res, Req, State).

to_term_default(Body, _Req, _State) -> decirest_validator:json_decode(Body).

validate_on_schema_fun(SchemaCallback) ->
    fun({Term, Req, State = #{module := Module}}) ->
       Res = case is_exported(Module, SchemaCallback, 0) of
                 true ->
...
validate_term({Term, Req, State = #{module := Module}}) ->
    Res = decirest:apply_with_default(Module,
                                      validate_term,
                                      [Term, Req, State],
                                      fun validate_term_default/3),
    epipe_resp(Res, Req, State).

validate_term_default(Term, _Req, _State) -> {ok, Term}.

validate_action({Term, Req, State = #{module := Module}}) ->
    Res = decirest:apply_with_default(Module,
                                      validate_action,
                                      [Term, Req, State],
                                      fun validate_action_default/3),
    epipe_resp(Res, Req, State).

validate_action_default(Term, _Req, _State) -> {ok, Term}.

%% Return same output as input and not put that requirement on callbacks
epipe_resp({ok, Res}, Req, State) -> {ok, {Res, Req, State}};
...
persist_data(Body, Req, State = #{module := Module}) ->
    case is_exported(Module, persist_data, 3) of
        true -> Module:persist_data(Body, Req, State);
        false -> Module:persist_data(Body, State)
    end.

perform_action(Body, Req, State = #{module := Module}) ->
    case is_exported(Module, perform_action, 3) of
        true -> Module:perform_action(Body, Req, State);
        false -> Module:perform_action(Body, State)
    end.
    ...
    case decirest:do_callback(Module, options, Req, State, fun options_default/2) of
        ...
        {ok, Req1, State1} ->
            case cowboy_req:has_resp_header(<<"allow">>, Req1) of
                true -> {ok, Req1, State1};
                false -> add_default_allow_header(Req1, State1)
            end
    end.

to_csv(_Name, Map) ->
    lists:map(fun({K, V}) -> make_csv_row([decirest:t2b(K), ";"], V) end, maps:to_list(Map)).

make_csv_row(Prefix, Map) when is_map(Map) ->
    lists:map(fun({K, V}) -> make_csv_row([Prefix, decirest:t2b(K), ";"], V) end,
              maps:to_list(Map));
make_csv_row(Prefix, List) when is_list(List) ->
    lists:map(fun(V) -> make_csv_row(Prefix, V) end, List);
make_csv_row(Prefix, Value) -> [Prefix, decirest:t2b(Value), <<"\n">>].
...
get_schema(Type, Module) ->
    case Module:output_schema() of
        #{Type := Schema} when is_map(Schema) -> Schema;
...
from_multi_default(Req, State) ->
    ...
    {file, Input, Filename, ContentType} = cow_multipart:form_data(Headers),
