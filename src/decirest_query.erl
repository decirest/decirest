...
get_bindings(Req, #{module := Module} = _State) ->
    ...
    DefaultQueryMatch =
        [{sort_by, [], undefined}, {render, fun decirest_query:bool/2, undefined}],
    QueryMatch =
        case erlang:function_exported(Module, query_bindings, 0) of
            false -> [];
            true -> Module:query_bindings()
        end,
...
check_for_duplicate_bindings(QueryBindings, Bindings, MergedBindings) ->
    case maps:size(QueryBindings) + maps:size(Bindings) == maps:size(MergedBindings) of
        ...
        false ->
            QueryBindingsSet =
                sets:from_list(
                    maps:keys(QueryBindings)),
            BindingsSet =
                sets:from_list(
                    maps:keys(Bindings)),
            DuplicateBindings =
                sets:to_list(
                    sets:intersection(QueryBindingsSet, BindingsSet)),
            lager:error("Same binding was used for both url binding and query_bindings: ~p",
                        [DuplicateBindings])
    end.
...
bool(forward, Value) ->
    case string:lowercase(Value) of
        ...
        _ ->
            {ok,
             false}  % default if any other than <<"true">> instead of getting a 400 if not <<"true">> or <<"false">>
    end;
bool(reverse, Value) ->
...
query_bindings_test() ->
    ...
    meck:expect(query_module,
                query_bindings,
                0,
                [{timestamp_from, [], 1}, {timestamp_to, [], 1000}]),
    Req = #{bindings => #{a => <<"cowboybinding">>},
            qs => <<"timestamp_from=16457000000000&timestamp_to=16467000000000">>},
