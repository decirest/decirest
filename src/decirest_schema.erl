...
schema_to_md(Json) ->
    [[<<"Schema\n---\n">>,
      pretty_prop({undefined, Json},
                  #{level => 0,
                    required => []})]].

pretty_prop({Property, Json},
            #{level := Level,
              required := Required}) ->
    ...
    DescriptionMd =
        get_description_md(maps:get(<<"description">>, Json, undefined), Indentation),
...
get_required_properties(_, Json) -> maps:get(<<"required">>, Json, []).

get_properties(<<"array">>, Json) ->
    maps:get(<<"properties">>, maps:get(<<"items">>, Json, #{}), #{});
get_properties(_, Json) -> maps:get(<<"properties">>, Json, #{}).
...
get_title_md(Property, _Json, Required) ->
    case lists:member(Property, Required) of
        false -> [<<"**">>, Property, <<"**">>, <<"\n">>];
        true -> [<<"**">>, Property, <<"** ">>, <<" *<required>required</required>*">>, <<"\n">>]
    end.
