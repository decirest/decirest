...
handle_body(Body, Req = #{method := Method}, State = #{module := Module}) ->
    ...
    case {Method, decirest_handler_lib:is_exported(Module, action_schema, 0)} of
        {<<"POST">>, true} -> validate_action(Body, Req, State);
        _ ->
...
validate_payload(Body, Req = #{method := Method}, State) ->
    ...
    case decirest_handler_lib:validate_payload(Body,
                                               Req,
                                               State#{method => Method,
                                                      module_binding => MB})
    of
...
-spec delete_resource_default(map(),
                              #{module := atom(),
                                _ => _}) ->
                                 {true | false, map(), map()}.
delete_resource_default(Req, State) -> decirest_handler_lib:delete_data(Req, State).
...
content_types_provided(Req, State = #{module := Module}) ->
    decirest:do_callback(Module,
                         ...
                         Req,
                         State,
                         fun content_types_provided_default/2).

-spec content_types_provided_default(_, _) ->
                                        {[{{_, _, _}, to_fun | to_html | to_json}, ...], _, _}.
...
to_html_default(Req, State = #{module := Module}) ->
    ...
    Title = Module:name(),
    Context = [{title, Title}, {single_data, Json}],
...
to_json_default(#{path := Path} = Req, State = #{module := Module}) ->
    ...
    ChildUrls =
        decirest:child_urls_map(
            decirest:get_children(Module), Req, State),
    PrettyConfig = decirest_handler_lib:maybe_pretty(Req, State),
    {jiffy:encode(
         maps:merge(ChildUrls, Data2#{self_url => Path}), [force_utf8] ++ PrettyConfig),
     Req,
     State}.
...
-spec resource_exists_default(_,
                              #{module := _,
                                _ => _}) ->
                                 any().
resource_exists_default(Req,
                        State =
                            #{mro_call := true,
                              module := Module}) ->
    case decirest_handler_lib:fetch_data(Req, State) of
        {ok, [Data]} -> decirest_auth:gate2(Req, decirest:put_data(Module, Data, State));
        {ok, []} -> {false, Req, State};
