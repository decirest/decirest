    ...
    case jesse:validate_with_schema(Schema, Obj, [{allowed_errors, infinity}]) of
        ...
        {error, Errors} ->
            case Options of
                #{strict := false} ->
                    ExtraProperties =
                        [Path || {data_invalid, _, no_extra_properties_allowed, _, Path} <- Errors],
                    validate_on_schema(drop_extra_properties(ExtraProperties, Obj),
                                       Schema,
                                       Options#{strict => true});
                _ -> {error, prettify_errors(Errors)}
            end
    end.
...
drop_extra_properties([], Obj) -> Obj.

-spec drop_extra_property([any(), ...], map()) -> map().
drop_extra_property([PathPart], Obj) -> maps:without([PathPart], Obj);
...
prettify_errors(Errors) -> prettify_errors(Errors, #{}).

-spec prettify_errors([{data_invalid, _, _, _, _}], _) -> any().
prettify_errors([{data_invalid,
                  #{<<"required">> := Required},
...
prettify_errors([], ErrorMap) -> ErrorMap.
...
add_required_errors([R | Required], Value, Path, ErrorMap) ->
    case Value of
        R ->
            P = get_path(Path ++ [R]),
            add_required_errors(Required,
                                Value,
                                Path,
                                ErrorMap#{P => [required | maps:get(P, ErrorMap, [])]});
        _ -> add_required_errors(Required, Value, Path, ErrorMap)
    end;
add_required_errors([], _Value, _Path, ErrorMap) -> ErrorMap.

-spec get_path([any()]) -> any().
get_path([PathPart | PathParts]) -> get_path(PathParts, PathPart);
get_path([]) -> <<"root">>.
...
validate_values(ReqMap, Map) ->
    ...
    case WrongValueList of
        [] -> ok;
        _ -> {error, format_faulty_values(WrongValueList, Map)}
    end.

format_faulty_values(ReqList, Map) ->
    maps:from_list(
        lists:map(fun({ReqK, ReqV}) ->
                     {ReqK,
                      #{req => ReqV,
                        value => maps:get(ReqK, Map)}}
                  end,
                  ReqList)).
