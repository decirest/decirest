...
fetch_doc_by_module(Ref, Module, Req, State) ->
    case get_all_model_routes(Ref) of
        #{Module := [FirstPath | _]} -> fetch_doc_by_path(Ref, Req#{path => FirstPath}, State);
        _ -> stop
    end.
...
fix_content_type(List) -> [<<A/binary, "/", B/binary>> || {{A, B, _}, _} <- List].

make_presentation_path(Path) -> make_presentation_path(Path, <<"/">>).
...
make_presentation_path([], Pth) -> Pth.

resource_path_fold({Path, Module}, Acc) ->
    case Acc of
        #{Module := Paths} -> Acc#{Module => [Path | Paths]};
        _ -> Acc#{Module => [Path]}
    end.
