...
init_default(Req, State) ->
  {cowboy_websocket, Req, State#{inital_req => Req, rstate => #{}}}.

websocket_init(State = #{module := Module}) ->
  decirest:apply_with_default(Module, websocket_init, [State], fun websocket_init_default/1).
    ...
    {ok, DecodedMsg} ->
      case Module:websocket_handle(DecodedMsg, State) of
        {ok, State} ->
          {ok, State};
        {reply, Res, NewState} ->
          {[{binary, term_to_binary(Res, [compressed])}], NewState}
      end;
