...
authenticate(Req,
             State =
                 #{module := Module,
                   decirest_auth_module := AuthModule}) ->
    decirest:apply_with_default(Module,
                                authenticate,
                                [Req, State],
                                fun AuthModule:authenticate/2);
authenticate(Req, State) -> {true, Req, State}.
...
gate1(Req,
    ...
    case maps:get(main_module, State) of
        ...
        _ ->
            ...
            {B, NewReq, NewState} =
                decirest:apply_with_default(Module, gate1, [MroReq, State], fun AuthModule:gate1/2),
            {B, decirest_req:set_method(NewReq, Method), NewState}
    end;
gate1(Req, State) -> {true, Req, State}.
...
gate2(Req,
    ...
    case maps:get(main_module, State) of
        ...
        _ ->
            ...
            MroReq = decirest_req:set_method(Req, <<"GET">>),
            {B, NewReq, NewState} =
                decirest:apply_with_default(Module, gate2, [MroReq, State], fun AuthModule:gate2/2),
            {B, decirest_req:set_method(NewReq, Method), NewState}
    end;
gate2(Req, State) -> {true, Req, State}.
...
gate3(Req, State) -> {true, Req, State}.
